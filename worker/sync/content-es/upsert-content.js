...
const {
	...
	SLACK: { INCOMING_HOOK_URL_T9N: SLACK_INCOMING_HOOK_URL_T9N },
} = require('config');
...
module.exports = exports = async (event, message, response, subscriber) => {
	try {
		...
		const CONTENT_STATE = String(eventName)
			.toLowerCase()
			.includes('create')
			? 'created'
			: 'deleted';
		...
		switch (CONTENT_STATE) {
			case 'created':
				...
				content_es.content_area =
					content_es.isWeekendContent === true
						? 'Spanish weekend'
						: 'Spanish content';
				...
				try {
					const content_en = await esClient.get(content_es.content_id);

					if (!content_en) {
						throw new Error(
							`ElasticSearch could not find content with ID: ${content_es.content_id}`
						);
					}
					...
					content_es.published_date = new Date(
						content_en.firstPublishedDate ||
							content_en.publishedDate
					);
					...
					log.info(
						`${MODULE_ID} UPSERTING => `,
						JSON.stringify(content_es, null, 4)
					);
				} catch (error) {
				...
				break;
			case 'deleted':
				await db.syndication.delete_content_es([
					path.basename(FILE_NAME, path.extname(FILE_NAME)),
				]);

				log.info(
					`${MODULE_ID} DELETING => ${path.basename(
						FILE_NAME,
