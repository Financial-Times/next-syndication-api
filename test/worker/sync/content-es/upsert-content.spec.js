...
describe(MODULE_ID, function () {
	...
	beforeEach(function () {
		...
		S3Object = require(path.resolve(`${FIXTURES_DIRECTORY}/translations/b6e54ea4-86c4-11e7-8bb1-5ba57d47eff7.json`));
		S3Promise = { promise: () => Promise.resolve({
			Body: Buffer.from(JSON.stringify(S3Object), 'utf8')
		}) };
		S3Stub = sinon.stub(__proto__, 'getObject').callsFake(() => S3Promise);

		content = require(path.resolve(`${FIXTURES_DIRECTORY}/content/b6e54ea4-86c4-11e7-8bb1-5ba57d47eff7.json`));
	...
	describe('S3 `ObjectDelete:*`', function() {
		it('flags the item as `deleted` in the DB', async function () {
			const event = require(path.resolve(`${FIXTURES_DIRECTORY}/s3-events/b6e54ea4-86c4-11e7-8bb1-5ba57d47eff7.delete.json`));
			const message = { data: event };

			await underTest(event, message, {}, subscriber);

			expect(db.syndication.delete_content_es).to.have.been.calledWith([event.Records[0].s3.object.key]);
		});
	});

	describe('S3 `ObjectCreated:*`', function() {
		it('retrieves the correct file from the S3 bucket', async function () {
			const event = require(path.resolve(`${FIXTURES_DIRECTORY}/s3-events/b6e54ea4-86c4-11e7-8bb1-5ba57d47eff7.create.json`));
			const message = { data: event };
		...
		it('retrieves the content from elastic search', async function () {
			...
			expect(getContentStub).to.have.been.calledWith(event.Records[0].s3.object.key);
		});
