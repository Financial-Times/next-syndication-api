...
module.exports = exports = async (contract_id) => {
	...
	const alreadyDownloaded = await db.query('SELECT * FROM syndication.get_downloads_by_contract_id($1::text)', [contract_id]);

	const alreadySaved = await db.query('SELECT * FROM syndication.get_saved_items_by_contract_id($1::text)', [contract_id]);
