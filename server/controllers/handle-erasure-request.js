...
module.exports = exports = async (req, res) => {
	...
	try {
		...
		log.error('GDPR_ERASURE_REQUEST_FAILED_TO_GET_USER_DATA', {
			error,
		});
	...
	try {
		...
		if (result?.anonymise_user_subject_data?.data?.anonymised) {
			...
			log.info('GDPR_ERASURE_REQUEST_PROCESSED', {
				counts,
				anonymised_user_id,
			});
			...
			log.warn('GDPR_ERASURE_REQUEST_FAILED_TO_ANONYMISE_USER_DATA', {
		...
		log.error('GDPR_ERASURE_REQUEST_FAILED_TO_ANONYMISE_USER_DATA', {
