...
module.exports = exports = async (req, res, next) => {
	const log = new Logger({
		req,
		res,
		source: 'middleware/get-contract-by-id-from-session',
	});
	const { locals } = res;
	const {
		$DB: db,
	...
	try {
		if (MAINTENANCE_MODE !== true) {
			const contract = (locals.contract = await getContractByID(
				syndication_contract.id,
				locals
			));

			locals.allowed = locals.contract.allowed = contract.items.reduce(
				(acc, { assets }) => {
					[
						['ft.com', 'ft_com'],
			...
			if (MASQUERADING !== true) {
				await db.syndication.upsert_contract_users([
					syndication_contract.id,
					user.user_id,
					contract.owner_email === user.email,
				]);
			}
		}
