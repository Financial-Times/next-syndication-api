...
**next-syndication-api** is owned by Customer Lifecycle and runs as **server**, **sync worker**, and **cron worker** components. It uses **Syndication AWS Postgres RDS**, **AWS SQS queues**, and **AWS S3 buckets** for persistent data, event processing, and object storage.
...
**next-syndication-api** validates the **Syndication user** session with **next-session**, gets licence data from **Access Licence Service**, requests a token from **API Authorization Service**, gets profile data from **User Profile API**, and writes user/contract-user records to **Syndication AWS Postgres RDS**.
